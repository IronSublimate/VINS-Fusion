...
def write_euroc_trajectory_file(file_path, traj: PoseTrajectory3D,
    ...
    if isinstance(file_path, str) and confirm_overwrite:
        if not file_interface.user.check_and_confirm_overwrite(file_path):
            return
    ...
    if isinstance(file_path, str):
        file_interface.logger.info("Trajectory saved to: " + file_path)
...
if __name__ == "__main__":
    ...
    write_euroc_trajectory_file(args.trajectory_out, trajectory)