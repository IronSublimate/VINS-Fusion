...
GlobalOptimization::GlobalOptimization(bool multi_thread) : multi_thread(multi_thread) {
    initGPS = false;
    newGPS = false;
...
void GlobalOptimization::GPS2XYZ(double t, double latitude, double longitude, double altitude, double *xyz) {
    if (!initGPS) {
        geoConverter.Reset(latitude, longitude, altitude);
        {
            Eigen::Matrix4d WVIO_T_body = Eigen::Matrix4d::Identity();
            Eigen::Matrix4d WGPS_T_body = Eigen::Matrix4d::Identity();
            assert(localPoseMap.find(t) != localPoseMap.cend());
...
void GlobalOptimization::inputGPS(double t, double latitude, double longitude, double altitude, double posAccuracy) {
    double xyz[3];
    GPS2XYZ(t, latitude, longitude, altitude, xyz);
...
void GlobalOptimization::optimize_once() {
    if (newGPS) {
        newGPS = false;
